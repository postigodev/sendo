    ...
    <form class="form hotkey-form" id="binding-form">
      ...
      <section class="hotkey-form-block">
        ...
        <label class="settings-field">
          ...
          <select class="settings-input" id="binding-action-type">${bindingActionOptions()}</select>
        </label>
        ${bindingActionRequiresValue(newBindingActionType) ? `<label class="settings-field"><span class="settings-label">${escapeHtml(bindingActionValueLabel(newBindingActionType))}</span>${bindingActionControl()}</label>` : ""}
      </section>
...
function bindingActionControl() {
  if (newBindingActionType === "fire_tv_key") {
    ...
    return `<select class="settings-input" id="binding-action-value">${values.map((value) => `<option value="${value}" ${newBindingActionValue === value ? "selected" : ""}>${value}</option>`).join("")}</select>`;
  }
  return `<select class="settings-input" id="binding-action-value"><option value="" ${newBindingActionValue ? "" : "selected"}>Choose cached app</option>${currentFireTvApps.map((app) => `<option value="${escapeHtml(app.package_name)}" ${newBindingActionValue === app.package_name ? "selected" : ""}>${escapeHtml(app.display_name)}</option>`).join("")}</select>`;
}
